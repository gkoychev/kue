...
Job.prototype.remove = function(fn){
  this.removeState();
  //DISABLE SEARCH getSearch().remove(this.id);
...
Job.prototype.state = function(state){
  ...
  if (uid!==false)
      client.hset('q:jobs:' + state + ':uniq', uid, 1);
...
Job.prototype.findByUID = function(uid, state, fn){
    var client = this.client;
    client.hget('q:jobs:' + state + ':uniq', uid, fn);
};
...
Job.prototype.save = function(fn){
  ...
  if (uid!==false) {
      this.findByUID(uid, 'inactive', function(err, found){
          if (err) return fn(err);
          if (found==1) {
              return fn(true, 'duplicate by UID');
          }
          else {
              prepareNewJob(self, fn);
          }
      });
  } else {
      prepareNewJob(self, fn);
...
Job.prototype.update = function(fn){
  ...
  // search data
  //DISABLE SEARCH getSearch().index(json, this.id);
};
